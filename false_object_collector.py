...
Opens a webcam stream or processes a video file, detects objects with YOLO,
and saves frames with detections in YOLO format for negative example training.
"""
import cv2
import numpy as np
import os
...
def run_video_false_object_collector(video_path, confidence_threshold=DEFAULT_CONFIDENCE_THRESHOLD):
    """
    Process a video file to collect false object data.
    ...
    # Set a slower frame display rate for better visualization (regardless of original FPS)
    display_fps = min(30, max(10, fps))  # Between 10-30 FPS for display
    ...
    while True:
        ...
        if frame_idx % 2 != 0:
            ...
            if pending_save_frame is not None:
                ...
                # Show progress info on the frame
                info_frame = frame.copy()
                cv2.putText(info_frame, f"Frame: {frame_idx}/{frame_count}", 
                           (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
                cv2.imshow('False Object Collector', info_frame)
        ...
        # Add frame info, counter and threshold to the frame
        cv2.putText(display_frame, f"Frame: {frame_idx}/{frame_count}", 
                   (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
        cv2.putText(display_frame, f"Saved: {saved_count}", (10, 60), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
        cv2.putText(display_frame, f"Threshold: {confidence_threshold}", (10, 90), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2)
        ...
        # Show the frame with detections
        cv2.imshow('False Object Collector', display_frame)
        key = cv2.waitKey(delay) & 0xFF  # Use calculated delay for better playback control
        if key == ord('q'):
            break
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode", "-m", type=str, choices=["camera", "video"], default="camera",
                        help="Input mode: 'camera' for live stream or 'video' for video file processing")
    ...
    if args.mode == "video":
        if not args.video:
            ...
            exit(1)
        run_video_false_object_collector(args.video, args.confidence)
    else:
